...
export default class Heap<T> {
    ...
    private heapifyUp() {
        ...
        while(currentIdx>0){
            ...
            //현재 노드의 값과 현재 노드의 부모값을 compare 함수에 넣어서 true면 교체
    ...
    /**
     * poll 이후에 수정된 최상단 노드가 올바른 자리로 찾아가게 하는 함수
     */
    private heapifyDown() {
        let currentIdx = 0;
        ...
        while(this.getLeftChildIndex(currentIdx)< count) {
            ...
            if(this.arr[rightChildIdx]) {
                targetIdx = this.compare(this.arr[rightChildIdx],this.arr[leftChildIdx])? rightChildIdx : leftChildIdx;
            }
