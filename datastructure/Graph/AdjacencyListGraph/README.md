# 연결리스트로 그래프 구현
## 연결리스트로 구현한 그래프
각 정점이 어떤 정점과 인접하지 리스트의 형태로 표현한 것이다. 각 정점마다 하나의 리스트를 가지고 있다. 각 리스트는 자신과 인접한 다른 정점을 담고 있다. 노드들의 관계를 연결리스트로 표현하는 방법
## 연결리스트 ADT
객체: 정점의 집합과 간선의 집합
필드
...
- addEdge(vertext1, vertext2): 그래프에서 vertext1과 vertext2연결
- removeVertext(vertext): 그래프에서 vertext 를 삭제
...
dfs(vertex:string){
    ...
    while(stack.length>0) {
        ...
        if(!current) continue;
        if(this.visitedVertex[current])continue;
        this.visitedVertex[current] = true;
        result.push(current);
        this.nodes.get(current)?.forEach((linkedVertex)=>{
            if(!this.visitedVertex[linkedVertex]){
...
bfs(vertex: string) {
    ...
    while(queue.length > 0){
        ...
        this.nodes.get(current)?.forEach((linkedVertex)=>{
            if(!this.visitedVertex[linkedVertex]){
                this.visitedVertex[linkedVertex] = true;
