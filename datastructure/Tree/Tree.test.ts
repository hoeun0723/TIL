...
describe('Tree',()=>{
    ...
    describe('addChildren Tree',()=>{
        ...
        it('addChildren은 인자로 Tree가 아닌 값을 받아야 한다.',()=>{
    ...
    describe('find Target in Tree',()=>{
        ...
        it('찾고자 하는 value를 입력하면 해당 node를 반환한다.',()=>{
            const res = new TreeNode('8',3);
            expect(tree.find('8')).toStrictEqual(res);
        });
        it('찾고자하는 node가 없으면 null을 반환한다.',()=>{
...
describe('delete Target InTree',()=>{
    let tree: TreeNode<string>;
    beforeEach(()=>{
        tree = makeExampleTree();
    });
    it('삭제하고자하는 value를 가진 Tree를 제거한한다.',()=>{
        tree.delete('6');
        expect(tree.find('6')).toBeNull();
    });
    it('해당 트리(3)의 자식 요소(3-1,3-2,3-3)이 없어야 한다.',()=>{
        tree.delete('3');
        expect(tree.find('3-1')).toBeNull();
        expect(tree.find('3-2')).toBeNull();
        expect(tree.find('3-3')).toBeNull();
    });
    it('입력한 값이 없으면 아무런 동작을 실행하지 않는다.',()=>{
        tree.delete('empty');
